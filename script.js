...
<span style='color:#89e1fd';> style</span>=\
<span style='color:#d28a68'>'padding: 1px; max-width:80%;'</span>\
<span style='color:#757575;'>&gt;</span></br></br>\
...
document.getElementById("title"+h3Count).addEventListener("keyup",function(){
    ...
    rightElement.innerHTML = "<center><h3>" + title.value + "</h3>" + "<p style='padding: 1px; max-width:80%;font-size:13px;'>" + content.value.replace(/\n/g, "") + "</p>";
    bottomElement.innerHTML = "&lt;h3 style='color: #000000; font-size: 18px; text-transform: none; font-weight: bold;'&gt;" + title.value + "&lt;/h3&gt;" + "&lt;p style='padding: 1px; max-width:80%;font-size:13px;'&gt;" + content.value.replace(/\n/g, "") + "&lt;/p&gt;";
